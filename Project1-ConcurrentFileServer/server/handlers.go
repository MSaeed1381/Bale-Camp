...
func (server *Server) DownloadFile(w http.ResponseWriter, r *http.Request) {
	...
	if len(fileId) == 0 {
		utils.JSONResponse(w, http.StatusBadRequest, Error{"file_id is empty"})
		return
	}

	file, _, err := server.fileHandler.DownloadFile(r.Context(), fileId)
	if err != nil {
		utils.JSONResponse(w, http.StatusBadRequest, Error{err.Error()})
		return
	}
	...
	if _, err := part.Write(file); err != nil {
		utils.JSONResponse(w, http.StatusInternalServerError, Error{err.Error()})
		return
	}
...
func (server *Server) UploadFormDataFile(w http.ResponseWriter, r *http.Request) {
	...
	fileId, err := server.fileHandler.UploadFile(r.Context(), fileData, utils.GetMimeTypeByExtension(extension))
	if err != nil {
		utils.JSONResponse(w, http.StatusBadRequest, Error{err.Error()})
		return
	}

	utils.JSONResponse(w, http.StatusOK, PostFile{fileId})
}

func (server *Server) UploadURLFile(w http.ResponseWriter, r *http.Request) {
	url := r.PostFormValue("file")
	if len(url) == 0 {
		utils.JSONResponse(w, http.StatusBadRequest, Error{"url is empty"})
	}
...
func (server *Server) Hello(w http.ResponseWriter, _ *http.Request) {
	utils.JSONResponse(w, http.StatusOK, Success{Message: "Welcome to Upload/Download Server"})
}
...
func (server *Server) ExistsFile(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := os.Stat(fmt.Sprintf("./files/%s", fileId)); errors.Is(err, os.ErrNotExist) {
		utils.JSONResponse(w, http.StatusNotFound, Error{Error: "file not found"})
		return
	}

	utils.JSONResponse(w, http.StatusOK, Success{Message: "file found!"})
}
